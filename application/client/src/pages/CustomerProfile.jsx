...
export default function CustomerProfile() {
  ...
  useEffect(() => {
    // Trying to fetch my own username that I creted
    // username: qwaszx
    // password: qwaszx1!
    const username = 'qwaszx'; 
    ...
    <div className="container mx-auto mt-10">
      <form
         ...
         <>
           ...
           <div className="mt-4 flex justify-center">
             ...
             <button
               ...
               onClick={toggleEditMode}>Cancel</button></div>
         </>
       ) : (
         <>
           {inputsUserProfile.map((input) => (
             <div key={input.id} className="mb-4"><label
                 htmlFor={input.name}
                 className="block text-gray-700 font-bold mb-2">
                 {input.label}
               </label>
               <div className="text-gray-900">{inputValues[input.name]}</div>
             </div>))}
           <div className="mt-4 flex justify-center">
             <button
               type="button"
       ...
       {errorMsg && (<div className="text-red-600 text-center mt-4">{errorMsg}</div>)}
         {htmlResponse && (<div className="text-red-600 text-center mt-4">{htmlResponse}</div>)}
     </form></div>);}
